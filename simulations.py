...
class Simulation:
    def __init__(self, num_nodes: int, prob_connection: float = 0.5, wealth_dist = None, wealth_to_income_fn = lambda x: x * 0.2, **wealth_kwargs):
        self.id_to_wealth = self.make_id_to_wealth_dict(num_nodes, wealth_dist, **wealth_kwargs)
        ...
        self.sorted_nodes = NetworkSort(self.G).sort()  # note this comes out at list of sets (in case of ties), not list of elements
        self.rank_correlation, self.best_rank_correlation_list = self.calculate_rank_correlation()
    ...
    def make_id_to_income_dict(self, wealth_to_income_fn):
        return {id: wealth_to_income_fn(wealth) for id, wealth in self.id_to_wealth.items()}


    def sort_ids_by_wealth(self):
        sorted_ids = sorted(self.id_to_wealth.keys(), key=lambda k: self.id_to_wealth[k], reverse=True)
    ...
    def make_graph(self, num_nodes, prob_connection=0.5):
        ...
        for pair in pairs:
            ...
            connected = np.random.choice([0, 1], p=[1 - prob_connection, prob_connection])
            if connected:
    ...
    def show(self):
        ...
        nx.draw(self.G, pos=positions, with_labels=True, node_color='skyblue', node_size=1500, font_size=12,
                font_color='black')
        edge_labels = {(u, v): self.G[u][v]['weight'] for u, v in self.G.edges()}
        nx.draw_networkx_edge_labels(self.G, pos=positions, edge_labels=edge_labels, font_color='black')
...
def run_simulations_partially_connected_no_noise():
    ...
    param_combinations = [(p1, p2) for p1 in num_nodes_values for p2 in prob_connected_values]

    # Run the function with each set of parameters and collect results
    results = []
    i = 1
    sims_per_combination = 3
    for param1, param2 in param_combinations:
        j = 0
        for j in range(sims_per_combination):
            update_str = str(i) + "/" + str(len(num_nodes_values) * len(prob_connected_values) * sims_per_combination)
            print(update_str)
    ...
    df_results.to_csv('simulation_results_partially_connected_no_noise.csv', index=False)
...
def run_simulations_all_edges_with_noise():
    ...
    for n in num_nodes_values:
        for j in range(sims_per_combination):
            update_str = str(i) + "/" + str(len(num_nodes_values) * sims_per_combination)
            print(update_str)

            simulation = Simulation(n, prob_connection=1, wealth_to_income_fn= lambda x: 0.2 * x + np.random.normal(scale=10000))
